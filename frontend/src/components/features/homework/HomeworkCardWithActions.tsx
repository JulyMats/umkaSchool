...
const HomeworkCardWithActions: React.FC<HomeworkCardWithActionsProps> = ({
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow flex flex-col h-full relative">
      <button
      ...
      <div className="flex items-start gap-3 mb-4">
        ...
        <div className="flex-1 min-w-0 pr-8">
          <h3 className="font-semibold text-lg truncate">{homework.title}</h3>
          <p className="text-gray-500 text-sm">Assigned by: {homework.teacherName}</p>
        </div>
      </div>
