...
export const statsService = {
    getStudentStats: async (studentId: string, period: TimePeriod = 'all'): Promise<StudentStats> => {
        ...
        const totalSeconds = filteredAttempts.reduce((sum, attempt) => {
            return sum + (attempt.timeSpentSeconds || 0);
        }, 0);

        // Calculate problems solved
        const problemsSolved = filteredAttempts.length;

        // Calculate accuracy rate
        const accuracySum = filteredAttempts.reduce((sum, attempt) => {
            return sum + (attempt.accuracy || 0);
        }, 0);
        const accuracyRate = problemsSolved > 0 ? Math.round(accuracySum / problemsSolved) : 0;
    ...
    getSubjectProgress: async (studentId: string, period: TimePeriod = 'all'): Promise<SubjectProgress[]> => {
        ...
        filteredAttempts.forEach(attempt => {
            ...
            stats.total++;
            stats.accuracySum += attempt.accuracy || 0;
            stats.timeSum += attempt.timeSpentSeconds || 0;
        });
