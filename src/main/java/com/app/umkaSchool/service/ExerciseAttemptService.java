...
public interface ExerciseAttemptService {
    ...
    List<ExerciseAttemptResponse> getExerciseAttemptsByStudentAndExercise(UUID studentId, UUID exerciseId);

    Long countAttemptsByStudent(UUID studentId);

    void deleteExerciseAttempt(UUID attemptId);

    ExerciseAttempt getExerciseAttemptEntity(UUID attemptId);
}